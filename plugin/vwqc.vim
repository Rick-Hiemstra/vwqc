...
def g:HelpMenu()
	var help_list = [             "NAVIGATION", 
					...
					":call AnnotationsReport(\"<tag>\", \"<attr filter>\")    Create tag annotations summary",
					":call QuotesReport(\"<tag>\", \"<attr filter\")          Create tag report for coded interview lines",
					":call Gather(\"<tag>\")                                Create secondary tag sub-report", 
					":call AllSummariesFull(\"<attr filter>\")              Create FullReport summaries for all tags in tag glossary", 
					":call AllSummariesQuotes(\"<attr filter>\")            Create QuotesReport summaries for all tags in tag glossary", 
					":call AllSummariesAnnos(\"<attr filter>\")             Create AnnotationsReport summaries for all interviews and all tags in tag glossary", 
...
def Annotation() 
	...
	if (current_line == match_line)
		...
		if (match_col == g:label_offset)	
			...
			silent! execute "normal! d$I**\<ESC>A**\<ESC>"
...
def g:GoToReference() 
	...
	var target_line1 = "Undefined"
	var target_line2 = "Undefined"
	# -----------------------------------------------------------------
	# Change the pwd to that of the current wiki.
	# -----------------------------------------------------------------
	execute "normal! :cd %:p:h\<CR>"
...
def GenSummaryLists(summary_type: string) 
	...
	for tag_index in range(0, (len(g:in_both_lists) - 1))
		if (len(g:attr_filter_list) == 0)
			file_name = "Summary " .. g:in_both_lists[tag_index] .. " - " .. summary_type .. " batch - filters - none" .. g:wiki_extension
			link_name = "[Summary " .. g:in_both_lists[tag_index] .. " - " .. summary_type .. " batch - filters - none](Summary " .. g:in_both_lists[tag_index] .. " - " .. summary_type .. " batch - filters - none)"
			g:summary_file_list = g:summary_file_list + [file_name]
			g:summary_link_list = g:summary_link_list + [link_name]
		else
			file_name = "Summary " .. g:in_both_lists[tag_index] .. " - " .. summary_type .. " batch - filters - " .. attr_filter_list_as_string .. g:wiki_extension
			link_name = "[Summary " .. g:in_both_lists[tag_index] .. " - " .. summary_type .. " batch - filters - " .. attr_filter_list_as_string .. "](Summary " .. g:in_both_lists[tag_index] .. " - " .. summary_type .. " batch - filters - " .. attr_filter_list_as_string .. ")"
			g:summary_file_list = g:summary_file_list + [file_name]
			g:summary_link_list = g:summary_link_list + [link_name]
		endif
	endfor
...
def g:CreateAndCountInterviewBlocks(search_term: string, ...attr_filter_list: list<string>)
	...
	for index in range(0, len(g:tags_list) - 1)
		...
		if (g:tags_list[index][2] == ':' .. search_term .. ':') && 
				(index(g:filtered_interview_list, g:current_interview) > -1) && (g:tag_filter_check == 1)
			if (g:tags_list[index][0] == g:last_interview)
				...
				#echom g:tag_count_dict[g:tags_list[index][0]][0] .. " last interview: " .. g:last_interview .. "\n"
...
def g:AttrTest(tag_item: string, ...attr_filter_list: list<string> )
	...
	execute "normal! :call g:AttrTest2(\"tag3\", " .. g:test .. ")\<CR>"
enddef
...
def g:ConvertAnnotationTopLine() 
	for annotation in range(0, (len(g:anno_list) - 1))
		...
		execute "normal gg"
		silent! execute 's/\://g'
		silent! execute "normal I**\<ESC>A**\<ESC>"
	endfor		
enddef
